...
파이어스토어 파일 저장에 적합X 사진, 동영상 같은 큰 파일은 파이어베이스 스토리지를 사용O

01-파이어베이스 스토리지 임포트하기

1."import "firebase/storage" 
2.파이어베이스 서비스를 임포트할 때는 항상 관련 서비스를 따로 묶음 후 임포트 
3.입력 후 "npm start"로 리액트 서비스 재 구동 필요 
4.파이어스토어와 다르게 문서에 아이디를 자동으로 만들지 않음 5.고유의 UUID를 만들어 스토리지에 저장

03-고유 식별자 UUID 라이브러리 설치

1.npm install uuid 
2.UUID 임포트 "import {v4 as uuid4 } from "uuid"; 
3.스토리지에 임포트 하기

06-스토리지에 사진 저장해 보기

1.스토리지 함수 putstring 함수 사용 
2.putstring 함수에 URL 인자로 전달하기만 하면 해당 파일이 스토리지에 바로 저장됨 
3.파이에베이스 스토리지에 저장된 파일 확인 가능

07-스토리지에서 사진 불러오기

1.response.ref.getDownloadURL() 함수 사용 
2.트윗 업로드 코드를 스토리지에서 사진을 저장하는 코드보다 뒤쪽에 작성하기 
3.if문 안에서 attachmentUrl을 정의하면 안됨

10-트윗 삭제 시 사진을 스토리지에서 삭제하기

1.스토리지에 불필요한 파일 저장 방지 
2.refFromURL 함수를 사용하면, attachmentUrl만으로 스트로지에서 해당 파일 위치 찾아 바로 삭제 가능
...
1-1. onChange 함수 입력할 때마다 value 인자로 키보드 값이 넘어감 newNweet에 반영하기 때문에 입력한 텍스트가 바로 보여짐.

1-2. 파이어스토어에 입력값 반영 새 입력값을 반영하기 위해 form 엘리먼터 & onSubmit 함수 필요 문서 자체를 수정하기 때문에 'doc(...).update(...)'로 작성 업데이트를 하고 싶은 값을 {text:newNweet}와 같은 객체 형태로 전달해야 업데이트가 제대로 실행.

2.사진 미리보기 기능
2-1 파일 첨부 양식 파일 업로드를 위해 'type="file"' 속성을 가진 input 엘리먼트 추가함. 파일 이벤트의 경우 event.target.value를 사용하면 파일 경로가 출력이 되지만 보안 정책 상 실제 경로를 표시하면 안되기 때문에 상세 경로가 숨겨진 상태로 출력. event.target.files와 같이 files 배열 참고.

files가 배열인 이유는 input 엘리먼트의 multiple 속성에 대비하기 위함. 여러 개의 이미지 파일 업로드 시 배열로 파일 정보를 보여주기 위함.

2-2 웹 브라우저에 사진 출력 웹 브라우저 api인 FileReader 사용 new FileReader()와 같이 new 키워드와 함께 사용 onloadend : readAsDataURL 함수에 전달한 파일이 함수로 들어간 이후의 결과값이 나온 다음 상황 감지.

3.사진 미리보기 기능 URL을 이용하여 파이어베이스에 사진을 저장하기 전 확인하는 기능 URL 관리를 위한 상태 필요
